...
class KsChatDelete(models.Model):
    ...
    def ks_delete_message(self, ks_message_id):
        ...
        res = self.env['res.users'].search([
            ('id', '=', self.browse(ks_message_id).create_uid.id)
        ])
        if self.env.user.has_group('base.group_system'):
            if ks_difference.seconds <= 3155692600:
                if res.has_group('base.group_system'):
                    custom_message = '<p><i>This message was deleted.</i></p>'
                else:
                    custom_message = '''<p><i>Admin has deleted this message.
                    </i></p>'''
...
class KsChatEnable(models.TransientModel):
    ...
    ks_chat_enable = fields.Boolean(
        config_parameter='base_setup.ks_chat_enable',
    )
    ks_admin_delete_access = fields.Boolean(
        config_parameter='base_setup.ks_admin_delete_access',
    )
...
class KsHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        rec['ks_chat_enable'] = self.env[
            'ir.config_parameter'
        ].sudo().get_param('base_setup.ks_chat_enable')
        rec['ks_admin_delete_access'] = self.env[
            'ir.config_parameter'
        ].sudo().get_param(
