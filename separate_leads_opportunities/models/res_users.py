...
class Users(models.Model):
        ...
        while crm_lead_index < 0 and index < len(result):
            ...
            if item['model'] == 'crm.lead':
                ...
                if item['model'] == 'res.partner':
                    item['icon'] = '''/separate_leads_opportunities/
                    static/description/contacts.png'''
                index += 1
    ...
    def _activity_from_crm_lead_user_count(self):
        ...
        self.env.cr.execute(query, (
            self.env.uid, self.env.uid, self.env.user.partner_id.id))
        ...
        # view_id_lead_search =
        # self.env.ref('sdi_crm.view_crm_case_leads_filter').id
        view_id_lead_search = self.env.ref('crm.view_crm_case_leads_filter').id
        # view_id_oppor_form =
        # self.env.ref('sdi_crm.crm_case_form_view_oppor').id
        view_id_oppor_form = self.env.ref('crm.crm_case_form_view_oppor').id
        # view_id_oppor_kanban =
        # self.env.ref('sdi_crm.crm_case_kanban_view_leads').id
        view_id_oppor_kanban = \
            self.env.ref('crm.crm_case_kanban_view_leads').id
        view_id_oppor_search = \
            self.env.ref('crm.view_crm_case_opportunities_filter').id
        ...
        for activity in activity_from_crm_lead_data:
            if not user_activities.get(activity['model']):
                ...
                if activity['model'] == 'lead':
                    ...
                    user_activities[activity['model']]['form_id'] = \
                        view_id_lead_form
                    user_activities[activity['model']]['kanban_id'] = \
                        view_id_lead_kanban
                    user_activities[activity['model']]['search_id'] = \
                        view_id_lead_search
                else:
                    user_activities[activity['model']]['icon'] = \
                        '''/separate_leads_opportunities/
