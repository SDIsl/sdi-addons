...
class MailRenderMixin(models.AbstractModel):
    ...
    def remove_href_odoo(
        ...
        urlSettings = self.env['ir.config_parameter'].get_param(
            'mail_debrand.linkUrl') or 'google.com'
        urlTextSettings = self.env['ir.config_parameter'].get_param(
            'mail_debrand.linkText') or 'Dessarrollado con &hearts; por SDi'  
        enlace = etree.Element("a", href="https://"+str(urlSettings))
        enlace.set("style", "text-align:center")
        enlace.set("style", "text-decoration:none")
        enlace.set("style", "background-color:coral")
        enlace.set("style", "width:590px")
        enlace.text = str(urlTextSettings)
        if len(value) < 20:
            return value
        # value can be bytes type; ensure we get a proper string
        if type(value) is bytes:
            value = value.decode()
        has_odoo_link = re.search(r"<a\s(.*)odoo\.com",
                                  value, flags=re.IGNORECASE
                                  )
        if has_odoo_link:
            # We don't want to change what was explicitly added in the message
            # body, so we will only change what is before and after it.
            if to_keep:
            ...
            for part in to_change:
                ...
                for elem in odoo_anchors:
                    ...
                    if remove_parent and len(parent.getparent()):
                        # anchor <a href odoo has a parent powered by
                        ...
                        if parent.tag == "td":  # also here can be powered by
                            parent.getparent().remove(parent)
                            parent.set("style", "text-align:center")
                            parent.getparent().append(enlace)
                        else:
                            parent.remove(elem)
                            parent.set("style", "text-align:center")
                            parent.append(enlace)
                part = etree.tostring(
    ...
    def _render_template(
        ...
        :param str model: model name of records
            on which we want to perform rendering
        :param list res_ids: list of ids of records
            (all belonging to same model)
