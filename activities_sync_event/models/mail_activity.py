        ...
        if action['context'].get('default_res_id') and \
           action['context']['default_res_model'] == 'crm.lead':
            lead = self.env['crm.lead'].browse(
                int(action['context']['default_res_id']))
...
def _get_customer(self, context):
    customer = None
    if context['default_res_model'] == 'res.partner' and \
       context['default_res_id']:
        ...
        obj = self.env[context['default_res_model']].browse(
            context['default_res_id'])
        model = self.env[context['default_res_model']].with_context(
            active_test=False)
