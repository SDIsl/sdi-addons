...
class VerifactuXMLValidator:
    ...
    def _validate_cliente(self):
        ...
        fecha = self.invoice.date_invoice_operation or self.invoice.date_invoice
    ...
    def _validate_fechas(self):
        ...
        fecha_exp = invoice.date_invoice
        fecha_op = invoice.date_invoice_operation
    ...
    def _validate_recargo_equivalencia(self):
        invoice = self.invoice
        analyzer = VerifactuLineAnalyzer(invoice)
        fecha = invoice.date_invoice or fields.Date.today()
    ...
    @staticmethod
    def get_recargo_equivalencia_from_tax_ids(invoice_line_tax_ids): # OJO ESTO ES ALO NUEVO
        RE_TYPES = {
    ...
    @staticmethod
    def infer_tipo_rectificativa(invoice):
        """
        Determina el tipo de rectificativa según las reglas AEAT:
        - "I" = Sustitutiva (mismo total, corrige datos)
        - "S" = Por diferencias u otros ajustes
        Compatible con Odoo 10–18.
        """
        # Obtener la factura original según versión
        original = (
            getattr(invoice, "reversed_entry_id", False)
            or getattr(invoice, "refund_invoice_id", False)
        )
        if not original:
            return "S"  # por defecto (sin referencia original)

        # Recuperar importes con compatibilidad
        get_val = lambda obj, name: getattr(obj, name, 0.0) or 0.0
        base_orig = get_val(original, "amount_untaxed")
        tax_orig = get_val(original, "amount_tax")
        total_orig = get_val(original, "amount_total")

        base_new = get_val(invoice, "amount_untaxed")
        tax_new = get_val(invoice, "amount_tax")
        total_new = get_val(invoice, "amount_total")

        same_base = abs(base_orig - base_new) < 0.01
        same_tax = abs(tax_orig - tax_new) < 0.01
        same_total = abs(total_orig - total_new) < 0.01

        # Lógica AEAT
        if same_base and same_tax and same_total:
            # Misma base/impuesto/total → error fundado o sustitución de datos
            return "I"  # Sustitutiva
        else:
            # Diferencias monetarias → por diferencias
            return "S"
    ...
    def build_descripcion_operacion(invoice):
        ...
        productos = list({
            (getattr(line, "name", "") or getattr(line, "product_id", False) and getattr(line.product_id, "display_name", "") or "").strip()
            for line in getattr(invoice, "invoice_line_ids", [])
            if (getattr(line, "name", False) or getattr(line, "product_id", False))
        })
        ...
        if is_refund:
            ...
            # Compatibilidad Odoo 10–12 → usa refund_invoice_id
            if hasattr(invoice, "refund_invoice_id") and getattr(invoice, "refund_invoice_id", False):
                orig = invoice.refund_invoice_id
                num_orig = getattr(orig, "number", "") or getattr(orig, "name", "")
                date_orig = getattr(orig, "date_invoice", None)
                resumen += " de la factura %s" % (num_orig or "anterior")
                if date_orig:
                    try:
                        resumen += " emitida el %s" % date_orig.strftime("%d-%m-%Y")
                    except Exception:
                        pass

            # Compatibilidad Odoo 13+ → usa reversed_entry_id
            elif hasattr(invoice, "reversed_entry_id") and getattr(invoice, "reversed_entry_id", False):
        ...
        if num_lines > 3:
            resumen += " (total %s conceptos)" % num_lines
    ...
    def compute_base_coste_proporcional(invoice, base_grupo, base_coste_total):
        ...
        base_total_factura = sum(
            ...
            if any("margen" in (tax.description or "").lower() for tax in line.invoice_line_tax_ids)
        )
