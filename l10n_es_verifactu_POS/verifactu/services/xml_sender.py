    ...
    def parse_response(self, response_text):
        ...
        try:
            ...
            if code and 20000 <= code < 21000:
                ...
                # --- Versión segura en Py2/Py3 ---
                desc_short = (desc_match.group(0).strip() if desc_match else "")[:120]

                # Forzar conversión a unicode en todos los componentes
                try:
                    desc_short = _u(desc_short)
                except Exception:
                    try:
                        desc_short = desc_short.decode('utf-8')
                    except Exception:
                        desc_short = u''

                try:
                    desc_fallback = _u(_("Consulta el detalle."))
                except Exception:
                    desc_fallback = u"Consulta el detalle."

                try:
                    msg_tpl = _("🛑 Error al enviar la factura a VeriFactu%s. %s")
                    msg_tpl = _u(msg_tpl)
                except Exception:
                    msg_tpl = u"🛑 Error al enviar la factura a VeriFactu%s. %s"

                try:
                    summary = msg_tpl % (
                        _u((" (código %s)" % code)) if code else u"",
                        desc_short or desc_fallback,
                    )
                except Exception:
                    # Fallback si aún fallase el formateo
                    summary = u"Error VeriFactu%s: %s" % (
                        (u" (código %s)" % code) if code else u"",
                        desc_short or desc_fallback,
                    )
