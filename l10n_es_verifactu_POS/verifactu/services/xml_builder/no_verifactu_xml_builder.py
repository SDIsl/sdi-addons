...
class VerifactuXMLBuilderNoVerifactu(object):
    ...
    def build(self):
        ...
        if tipo_factura in ("F2", "R5"):
            vat = (inv.partner_id.vat or u"").upper().strip()
            ...
            ET.SubElement(id_dest, ET.QName(NS_SUM1, "NombreRazon")).text = client_name or u"SINNOMBRE"

            idinfo = VerifactuXMLValidator.build_id_for_xml(inv.partner_id)
            if idinfo.get("tag") == "NIF":
                ET.SubElement(id_dest, ET.QName(NS_SUM1, "NIF")).text = idinfo.get("value", u"")
            else:
                _id = (idinfo.get("ID") or u"").strip()
                if not _id:
                    raise UserError(_("VeriFactu: el destinatario no tiene NIF válido ni ID alternativo (IDType/CodigoPais/ID)."))
                idotro = ET.SubElement(id_dest, ET.QName(NS_SUM1, "IDOtro"))
                ET.SubElement(idotro, ET.QName(NS_SUM1, "IDType")).text = idinfo.get("IDType", u"")
                ET.SubElement(idotro, ET.QName(NS_SUM1, "CodigoPais")).text = idinfo.get("CodigoPais", u"")
