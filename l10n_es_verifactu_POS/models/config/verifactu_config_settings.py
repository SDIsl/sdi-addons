...
# Adaptado a Odoo 11 — Desarrollado por Juan Ormaechea (Mr. Rubik) — Odoo Proprietary License v1.0
...
def _odoo_major():
    try:
        ...
        return 11  # v11 por defecto aquí


def _is_modern(env):
    """True para >= v13 (display_notification). En v11 es False → rainbow_man/UserError."""
    try:
...
def _notify_success(env, title, message, sticky=False):
    # v11: no display_notification → rainbow_man
    if _is_modern(env):
        return {
...
# ─────────── Clase principal (res.config.settings, v11) ───────────
class VerifactuResConfigSettings(models.TransientModel):
    ...
    # Histórico (solo lectura; compute sin inverse)
    ...
    # Carga de histórico (solo UI; no inverse)
    @api.depends()
    ...
    def _onchange_mode_toggle(self):
        """Solo UI: exclusividad y regla del año al intentar pasar a No VeriFactu (v11)."""
        if not self.id:
            return
        ...
        if self.no_verifactu_mode_enabled:
            ...
            last_mode = ICP.get_param('l10n_es_verifactu_POS.last_verifactu_mode')
            activation_date_str = ICP.get_param('l10n_es_verifactu_POS.verifactu_mode_activation_date')
    ...
    def set_values(self):
        ...
        for rec in self:
            rec._vf_write_config_from_self()
    ...
    def _vf_get_values_dict(self):
        # Siempre company-aware
        company = self._company()
        Config = self.env['verifactu.endpoint.config'].sudo()
        config = Config.with_context(force_company=company.id).get_singleton_record()

        Lic = self.env["verifactu.license"].sudo().get_singleton_record()
        ICP = self.env['ir.config_parameter'].sudo()

        # Opcional: claves namespaced por compañía (no obligatorias para la UI)
        def _icp_get_ns(key):
            ns_key = "l10n_es_verifactu_POS.%s.%s" % (company.id, key)
            return ICP.get_param(ns_key)
        ...
        # (Opcional) espejo ICP namespaced
        ns_mode = _icp_get_ns('last_verifactu_mode')
        ns_date = _icp_get_ns('verifactu_mode_activation_date')
        res['last_verifactu_mode'] = res['last_verifactu_mode'] or ns_mode
        res['verifactu_mode_activation_date'] = res['verifactu_mode_activation_date'] or ns_date
        return res

    @api.multi
    def _vf_write_config_from_self(self):
        """Persistencia por compañía + espejo ICP namespaced (opcional)."""
        self.ensure_one()
        company = self._company()
        ...
        def _icp_set_ns(key, value):
            ns_key = "l10n_es_verifactu_POS.%s.%s" % (company.id, key)
            ICP.set_param(ns_key, value if value is not False else "")

        # --- calcula modo nuevo/previo ---
        ...
        # --- cambio de modo → persistir y espejo ICP ---
        if new_mode != prev_mode:
            ...
            _icp_set_ns('last_verifactu_mode', new_mode)
            _icp_set_ns('verifactu_mode_activation_date', now)
    ...
    def action_run_anomaly_scan(self):
        ...
        return _notify_success(self.env, _("Detector de anomalías"), _("Escaneo ejecutado."))
