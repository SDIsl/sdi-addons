    ...
    def _send(self, auto_commit=False, raise_exception=False,
        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                # remove attachments if user send the link with the
                # access_token
                ...
                mail.write({
                    'state': 'exception',
                    'failure_reason': _('Error without exception. Probably due'
                                        ' do sending an email without computed'
                                        ' recipients.'),
                })
                # Update notification in a transient exception state to avoid
                # concurrent update in case an email bounces while sending all
                # emails related to current mail record.
                ...
                if notifs:
                    notif_msg = _(
                        ...
                        'access update of notification records. Please see '
                ...
                for email in email_list:
                    ...
                    if mail.notification and not mail.email_to and \
                            mail.model in ['project.task', 'project.project']:
                        recipients_emails = mail.recipient_ids.mapped('email')
                        emailed_users = self.env['res.users'].search([
                            '|',
                            ('partner_id', 'in', mail.recipient_ids.ids),
                            ('partner_id.email', 'in', recipients_emails),
                        ])
                        internal_users = emailed_users.filtered(
                            lambda user: user.has_group('base.group_user'))
                        record = self.env[mail.model].search(
                            [('id', '=', mail.res_id)])
                        if len(emailed_users.ids) == len(internal_users.ids):
                            if subject == 'Re: %s' % (record.name):
                                ...
                                elif mail.model == 'project.project':
                                    subject = 'Re: %s - %s' % (
                                        record.partner_id.display_name,
                                        record.name
                                    )
                    ...
                    try:
                        ...
                        if str(error) == IrMailServer.NO_VALID_RECIPIENT:
                            ...
                            raise
                # mail has been sent at least once, no major exception occured
                if res:
                ...
                _logger.exception(
                    ...
                    'option',
                    mail.id, mail.message_id)
                # mail status will stay on ongoing since transaction will be
                # rollback
                raise
            except (psycopg2.Error, smtplib.SMTPServerDisconnected):
                # If an error with the database or SMTP session occurs, chances
                # are that the cursor or SMTP session are unusable, causing
                # further errors when trying to save the state.
                _logger.exception(
                    'Exception while processing mail with ID %r and Msg-Id '
                    '%r.',
                    mail.id, mail.message_id)
                raise
                ...
                _logger.exception(
                    'failed sending mail (id: %s) due to %s', mail.id,
                    failure_reason)
                mail.write(
                    {'state': 'exception', 'failure_reason': failure_reason})
                mail._postprocess_sent_message(
                    success_pids=success_pids, failure_reason=failure_reason,
                    failure_type='UNKNOWN')
                if raise_exception:
                    if isinstance(e, (AssertionError, UnicodeEncodeError)):
                        if isinstance(e, UnicodeEncodeError):
                            ...
                            # value and throw a MailDeliveryException that is
                            # an except_orm, with name and value as arguments
                            value = '. '.join(e.args)
